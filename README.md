...
See [source/method review](docs/METHOD_AND_SOURCE_REVIEW.md), [implementation plan](IMPLEMENTATION_PLAN.md), [pinned revisions](configs/sources.lock.json), and [actual execution handoff](reports/HANDOFF.md). Large data, score matrices and environments are ignored by Git. Actual run outputs live under `runs/` or a user-selected output path. The committed result summaries identify what ran and what remains unavailable.
...
```bash
.venv/bin/python -m preference_diff score --dataset imagereward \
...
.venv/bin/python -m preference_diff report --output "$PREFERENCE_RUN_ROOT/imagereward" --heatmaps 3
...
# End-to-end on a node with both network and an allocated GPU:
.venv/bin/python -m preference_diff run --dataset imagereward \
  --output runs/new_pilot --group-limit 100 --bootstrap 2000
```
